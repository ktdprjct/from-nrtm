...
let handler = async(m,{ conn, text }) => {
  if(!text) return m.reply('_LINKNYA MANA?_')
  m.reply('_sedang diproses..._')
  res = await TiktokDownloader(`${text}`).catch (e => {
    m.reply('Lagi error kak...')
    }
  )
  conn.sendFile(m.chat, `${res.result.audio}`, 'tiktokaudio.mp3', '', m)
  //conn.sendMessage(m.chat, `${res.result.audio}`, MessageType.audio, { quoted: m, mimetype: 'audio/mp4'})
}
